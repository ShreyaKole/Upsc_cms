...
import type { Paper, ExamResult } from '../../types';
import { Calendar, ArrowRight, Award, RotateCcw, BarChart2 } from 'lucide-react';
...
export const PaperCard: React.FC<PaperCardProps> = ({
  ...
  return (
    <div className="glass-card rounded-2xl p-6 flex flex-col justify-between transition-all duration-300 hover:shadow-2xl hover:shadow-indigo-500/10 hover:-translate-y-1 group relative overflow-hidden border border-slate-800/80">
      <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-indigo-500 via-cyan-400 to-indigo-600 opacity-80 group-hover:opacity-100 transition-opacity" />
      ...
      <div>
        <div className="flex items-center justify-between gap-2 mb-3">
          ...
          <span className="inline-flex items-center space-x-1 px-2 py-0.5 rounded text-[11px] font-medium bg-slate-800 text-slate-300">
            {paper.category}
          </span>
        </div>
      ...
      <div>
        ...
        <div className="flex gap-2">
          {latestResult ? (
            ...
            <button
              onClick={() => onStartTest(paper)}
              className="w-full flex items-center justify-center space-x-2 px-4 py-3 rounded-xl text-sm font-bold bg-gradient-to-r from-indigo-600 to-indigo-500 hover:from-indigo-500 hover:to-indigo-400 text-white shadow-lg shadow-indigo-600/25 transition-all transform active:scale-95"
            >
